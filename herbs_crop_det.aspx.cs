        ...
        protected void DataList1_ItemCommand(object sender, DataListCommandEventArgs e)
        {
            if (e.CommandName == "addtocart")
            {
                ...
                if (sizeVariantLabel != null && !string.IsNullOrEmpty(sizeVariantLabel.Text))
                {
                    ...
                    Response.Redirect("addtocart.aspx?crop_name=" + e.CommandArgument.ToString() + "&size=" + selectedSize + "&Quantity=" + txtbox.Text);
                }
                else
                {
            ...
            if (clickedButton != null)
            {
                ...
                if (item != null)
                {
                    ...
                    if (lblDisPrice != null && lblOrgPrice != null)
                    {
                        ...
                        if (clickedButton.CommandArgument == "40 ml")
                        {
                            // No multiplier needed for 10 gms
                        }
                        else if (clickedButton.CommandArgument == "80 ml")
                        {
                            disPrice *= 2; // Multiply by 10 for 100 gms size
                            orgPrice *= 2; // Multiply by 10 for 100 gms size
                        }
                        else if (clickedButton.CommandArgument == "120 ml")
                        {
                            disPrice *= 3; // Multiply by 50 for 500 gms size
                            orgPrice *= 3; // Multiply by 50 for 500 gms size
                        }
