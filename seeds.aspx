<%@ Page Title="" Language="C#" MasterPageFile="~/Master.Master" AutoEventWireup="true" CodeBehind="seeds.aspx.cs" Inherits="Final_Project.WebForm3" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
</asp:Content>
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <div class="head">
                <div class="description">
                    <asp:Label ID="descp" runat="server" Text="Seeds Online | Buy Best Quality Agricultural Seeds Online" />
                </div>
            </div>
            <div class="all_seeds">
                <%--<a href="vegetable_seeds_det.aspx">--%>
                <asp:DataList ID="DataList1" runat="server" RepeatDirection="Horizontal" RepeatColumns="5" Font-Bold="False" Font-Italic="False" Font-Overline="False" Font-Strikeout="False" Font-Underline="False" DataSourceID="SqlDataSource2" OnItemCommand="DataList1_ItemCommand">
                    ...
                    <div class="seeds_types">
                        <asp:HiddenField ID="HiddenField1" runat="server" Value='<%# Eval("seed_name") %>' />
                        <asp:HiddenField ID="HiddenField2" runat="server" Value='<%# Eval("category") %>' />
                        <asp:HiddenField ID="HiddenField3" runat="server" Value='<%# Eval("seed_brand") %>' />
                        <asp:ImageButton ID="ImageButton1" runat="server" ImageUrl='<%# Eval("seed_img") %>' width="220px" Height="180px" CssClass="image_style"  CommandName="view"  CommandArgument='<%# Eval("seed_id") + "," + Eval("seed_name") + "," + Eval("seed_brand") + "," + Eval("category") %>' />
                             <br />
                        <div class="details">
                        <asp:Label ID="Label1" runat="server" Text='<%# Eval("seed_name") %>' Font-Size="15px"></asp:Label>
                        <br />
                        <asp:Label ID="Label2" runat="server" Text='<%# Eval("seed_brand") %>' Font-Size="12px" ForeColor="#908D9D"></asp:Label>
                            <br />
                            <br />
                            ...
                            <br />
                            <br />
                            <asp:Button ID="Button1" runat="server" BackColor="Red" Text="Buy Now"  CommandName="view"  CommandArgument='<%# Eval("seed_id") + "," + Eval("seed_name") + "," + Eval("seed_brand") + "," + Eval("category") %>' />
                            <asp:Button ID="view" runat="server" Text="view details" CommandName="view" CommandArgument='<%# Eval("seed_id") + "," + Eval("seed_name") + "," + Eval("seed_brand") + "," + Eval("category") %>' />
                    </div>
                        <%--</a>--%>
                        </div>
                </ItemTemplate>
            </asp:DataList>
                <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString='<%$ ConnectionStrings:major_projectConnectionString4 %>' SelectCommand="SELECT * FROM [seeds]" DeleteCommand="DELETE FROM [seeds] WHERE [seed_id] = @seed_id" InsertCommand="INSERT INTO [seeds] ([seed_id], [seed_img], [seed_brand], [seed_name], [dis_price], [org_price], [size], [category]) VALUES (@seed_id, @seed_img, @seed_brand, @seed_name, @dis_price, @org_price, @size, @category)" UpdateCommand="UPDATE [seeds] SET [seed_img] = @seed_img, [seed_brand] = @seed_brand, [seed_name] = @seed_name, [dis_price] = @dis_price, [org_price] = @org_price, [size] = @size, [category] = @category WHERE [seed_id] = @seed_id">
                    <DeleteParameters>
                        <asp:Parameter Name="seed_id" Type="Int32"></asp:Parameter>
                    </DeleteParameters>
                    <InsertParameters>
                        <asp:Parameter Name="seed_id" Type="Int32"></asp:Parameter>
                        <asp:Parameter Name="seed_img" Type="String"></asp:Parameter>
                        <asp:Parameter Name="seed_brand" Type="String"></asp:Parameter>
                        <asp:Parameter Name="seed_name" Type="String"></asp:Parameter>
    ...
    <%--</div>--%>
            ...
            <div class="all_seeds">
                <%--<a href="vegetable_seeds_det.aspx">--%>
                <asp:DataList ID="DataList2" runat="server" RepeatDirection="Horizontal" RepeatColumns="5" Font-Bold="False" Font-Italic="False" Font-Overline="False" Font-Strikeout="False" Font-Underline="False" DataSourceID="SqlDataSource3" OnEditCommand="DataList2_EditCommand">
                    <ItemTemplate>
                    <div class="seeds_types">
                        <asp:HiddenField ID="HiddenField1" runat="server" Value='<%# Eval("seed_name") %>' />
                        <asp:HiddenField ID="HiddenField2" runat="server" Value='<%# Eval("category") %>' />
                        ...
                        <asp:Label ID="Label2" runat="server" Text='<%# Eval("seed_brand") %>' Font-Size="12px" ForeColor="#908D9D"></asp:Label>
                            ...
                            &nbsp; <span style="color: rgb(77, 81, 86);font-weight:bold; font-family: Arial, sans-serif; font-size: 16px; font-style: normal; font-variant-ligatures: normal; font-variant-caps: normal; font-weight: 400; letter-spacing: normal; orphans: 2; text-align: left; text-indent: 0px; text-transform: none; widows: 2; word-spacing: 0px; -webkit-text-stroke-width: 0px; white-space: normal; background-color: rgb(255, 255, 255); text-decoration-thickness: initial; text-decoration-style: initial; text-decoration-color: initial; display: inline !important; float: none;">₹</span><asp:Label ID="Label4" runat="server" Text='<%# Eval("org_price") %>' Font-Strikeout="True" ForeColor="#908D9D"></asp:Label>
                            <br />
                            <asp:DropDownList ID="DropDownList1" runat="server"  CssClass="no-expand" OnClick="openModal()" ViewStateMode="Inherit">
                                <asp:ListItem Value="10 gms">10 gms</asp:ListItem>
                            </asp:DropDownList>
                            <br />
                            <br />
                ...
                <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString='<%$ ConnectionStrings:major_projectConnectionString9 %>' SelectCommand="SELECT * FROM [seeds] WHERE ([all_category] = @all_category)">
                    <SelectParameters>
...
<asp:SqlDataSource ID="SqlDataSource4" runat="server" ConnectionString='<%$ ConnectionStrings:major_projectConnectionString9 %>' SelectCommand="SELECT * FROM [seeds]"></asp:SqlDataSource>
